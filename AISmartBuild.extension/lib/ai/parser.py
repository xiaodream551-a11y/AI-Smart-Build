...
_ACTION_ALIASES = {
    ...
    u"查询汇总": "query_summary",
    u"统计汇总": "query_summary",
    u"模型统计": "query_summary",
    "unknown": "unknown",
}
...
def parse_command(reply_text):
    ...
    raise ValueError(u"无法从回复中提取 JSON 指令：{}".format(
...
def dispatch_command(doc, command, levels=None):
    ...
    if not isinstance(normalized_command, dict):
        raise ValueError(u"命令必须为 JSON 对象")
    ...
    if action == "create_column":
        ...
        return params.get("message", u"无法理解你的指令，请换个说法试试")
    else:
        return u"不支持的操作类型: {}".format(action)
...
def normalize_command(command):
    ...
    if action == "batch":
        ...
        if not isinstance(commands, list) or not commands:
            raise ValueError(u"批量命令至少需要 1 条子指令")
        return {
            "action": "batch",
            "params": {
...
def _normalize_parsed_command_payload(payload):
    if isinstance(payload, list):
        if not payload:
            raise ValueError(u"JSON 指令数组不能为空")
...
def _normalize_params_by_action(action, params):
    ...
    if action in (
        ...
        floor = _first_present(
            normalized,
            ["floor", "level", "story", u"楼层", u"层", u"故事层"]
        )
    ...
    if action == "create_slab" and "boundary" not in normalized:
        boundary = _first_present(
            normalized,
            ["points", "vertices", "polygon", "boundary_points", u"边界", u"轮廓", u"顶点"]
        )
    ...
    if action == "modify_section":
        old_section = _first_present(
            normalized,
            ["old_section", "from_section", "source_section", u"旧截面", u"原截面"]
        )
        new_section = _first_present(
            normalized,
            ["new_section", "to_section", "target_section", u"新截面", u"目标截面"]
        )
    ...
    if action == "generate_frame":
        num_floors = _first_present(
            normalized,
            ["num_floors", "floors", "floor_count", "story_count", "stories", u"层数"]
        )
        floor_height = _first_present(
            normalized,
            ["floor_height", "story_height", "height", "standard_floor_height", u"层高", u"标准层层高"]
        )
        first_floor_height = _first_present(
            normalized,
            ["first_floor_height", "first_height", "ground_floor_height", u"首层层高"]
        )
        beam_section = _first_present(
            normalized,
            ["beam_section", "beam_size", "beam_section_default", u"梁截面"]
        )
        beam_section_x = _first_present(
            normalized,
            ["beam_section_x", "beam_x_section", u"x向梁截面", u"X向梁截面"]
        )
        beam_section_y = _first_present(
            normalized,
            ["beam_section_y", "beam_y_section", u"y向梁截面", u"Y向梁截面"]
        )
        column_section = _first_present(
            normalized,
            ["column_section", u"柱截面"]
        )
...
def _looks_like_generate_frame_request(text):
    ...
    if u"生成" in normalized_text and (u"跨" in normalized_text or u"层" in normalized_text):
        return True
    if re.search(u"\\d+\\s*跨", normalized_text):
        return True
    return False
...
def _exec_create_column(doc, params, levels):
    ...
    if not base_level or not top_level:
        return u"柱的楼层范围无效，可用边界编号为 1 到 {}".format(len(levels))

    create_column(doc, x, y, base_level, top_level, section)
    return u"已在 ({},{}) 创建 {} 柱，{} 层到 {} 层".format(
...
def _exec_create_beam(doc, params, levels):
    ...
    if not level:
        return u"楼层超出范围，可用楼层为 1 到 {}".format(get_story_count(levels))

    create_beam(doc, sx, sy, ex, ey, level, section)
    return u"已创建 {} 梁，从 ({},{}) 到 ({},{})，第 {} 层".format(
...
def _exec_create_slab(doc, params, levels):
    ...
    return u"已在第 {} 层创建楼板".format(floor)
...
def _exec_query_count(doc, params):
    ...
    name_map = {"column": u"柱", "beam": u"梁", "slab": u"板", "floor": u"板"}
    if floor is None:
        return u"当前模型中共有 {} 个{}构件".format(
            count, name_map.get(element_type, "")
        )
    return u"当前模型第 {} 层共有 {} 个{}构件".format(
...
def _collect_query_elements(doc, element_type, floor=None, section=None):
    ...
    if not cat:
        return None, None, u"不支持查询的构件类型: {}".format(element_type)
    ...
    if floor is not None:
        if not levels:
            return None, None, u"标高不足"

        floor_index = normalize_floor_number(floor)
        if floor_index is None:
            return None, levels, u"楼层参数无效: {}".format(floor)

        level = resolve_story_level_by_category(levels, cat, floor_index)
        if not level:
            return None, levels, u"楼层超出范围，可用楼层为 1 到 {}".format(get_story_count(levels))
...
def _format_query_detail_empty(element_type, floor=None, section=None):
    ...
    return u"{}未找到 {}".format(scope_text, target_text)
...
def _format_query_detail_line(DB, doc, levels, element, element_type):
    ...
    if element_type == "column":
        ...
        parts.append(u"位置 {}".format(_format_point_text(point)))
    elif element_type == "beam":
        curve = getattr(getattr(element, "Location", None), "Curve", None)
        if curve is not None:
            parts.append(u"起点 {}".format(_format_point_text(curve.GetEndPoint(0))))
            parts.append(u"终点 {}".format(_format_point_text(curve.GetEndPoint(1))))
    elif element_type == "slab":
        parts.append(u"面积 {} m²".format(format_number(get_element_area_sqm(DB, element))))

    if floor_label:
        parts.append(u"楼层 {}".format(floor_label))

    section_text = get_element_section_text(DB, element)
    if section_text and element_type in ("column", "beam"):
        parts.append(u"截面 {}".format(section_text))
...
def _format_query_summary_line(element_type, elements):
    ...
    if element_type == "slab":
        return u"{}：{} {}".format(label, count, unit)

    from pyrevit import DB

    section_counts = {}
    for element in elements:
        section_text = get_element_section_text(DB, element) or u"未标注截面"
    ...
    return u"{}：{} {}（{}）".format(label, count, unit, ", ".join(details))
...
def _resolve_element_story_label(DB, levels, element_type, element):
    ...
    for floor_number in range(1, get_story_count(levels) + 1):
        ...
        if getattr(getattr(level, "Id", None), "IntegerValue", -1) == target_level_id:
            return u"第 {} 层".format(floor_number)
    return ""
