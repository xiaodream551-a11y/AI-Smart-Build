...
2. 注册 pyRevit 扩展路径：
   将包含 `AISmartBuild.extension` 的目录加入 pyRevit 的自定义扩展搜索路径。可通过 pyRevit 设置界面添加，也可使用你本机已安装的 pyRevit CLI 完成配置。
...
选择 `AISmartBuild.extension/templates/构件导入模板.xlsx` 或同格式清单文件，批量导入柱、梁构件。
...
- `AISmartBuild.extension/AISmartBuild.tab/`：pyRevit Ribbon 按钮入口。
- `AISmartBuild.extension/lib/engine/`：共享建模引擎，封装 Revit API 操作。
- `AISmartBuild.extension/lib/ai/`：AI 能力接入、Prompt 模板与指令分发。
- `AISmartBuild.extension/templates/`：Excel 导入模板。
- `AISmartBuild.extension/startup.py`：扩展加载时输出版本与运行环境。
