...
def _normalize_failed_filter_state(source_filter_kind=None, action=None, keyword=None):
...
def _load_last_command_from_latest_conversation_log():
...
def _extract_last_command_from_conversation_markdown(content):
    entries = _extract_command_entries_from_conversation_markdown(content)
    if not entries:
        return None
    return entries[-1].get("command")


def _load_command_entries_from_latest_conversation_log():
    ...
    if latest_md_path:
        ...
        return _extract_command_entries_from_conversation_markdown(content)
...
def _load_last_failed_command_entry_from_latest_conversation_log():
    entries = _load_command_entries_from_latest_conversation_log()
...
def _load_last_failed_filter_from_latest_conversation_log():
    entries = _load_command_entries_from_latest_conversation_log()
    for entry in reversed(entries):
...
def _load_last_failed_selected_round_index_from_latest_conversation_log():
    entries = _load_command_entries_from_latest_conversation_log()
    for entry in reversed(entries):
        round_index = _normalize_failed_selected_round_index_from_entry(entry)
        if round_index is not None:
            return round_index
    return None


def _load_failed_command_entries_from_latest_conversation_log():
    entries = _load_command_entries_from_latest_conversation_log()
    return [
...
def _build_entry_from_turn(round_index, turn):
    ...
    return {
        ...
        "error": error_text,
        "error_summary": _summarize_error_text(error_text),
        "recovery_suggestion": recovery_text,
        "recovery_summary": _summarize_recovery_text(recovery_text),
        "failed_filter": _normalize_failed_filter_from_entry(turn),
        "failed_selected_round_index": _normalize_failed_selected_round_index_from_entry(turn),
        "command": command if isinstance(command, dict) else None,
    }


def _extract_command_entries_from_conversation_markdown(content):
    sections = re.findall(
        r"## 第 (\d+) 轮 \[(.*?)\](.*?)(?=\n## 第 \d+ 轮 \[|\Z)",
        content or "",
        re.DOTALL
    ...
    for round_index, _timestamp, body in sections:
        ...
        status = _normalize_status_label(
        ...
        entries.append(_build_entry_from_turn(int(round_index), {
            ...
            "failed_selected_round_index": _normalize_failed_selected_round_index(
...
def _normalize_failed_filter_from_entry(entry):
    ...
    if isinstance(failed_filter, dict):
        normalized = _normalize_failed_filter_state(
...
def _normalize_failed_selected_round_index(value):
    if value in (None, ""):
        return None
    try:
...
def _normalize_failed_selected_round_index_from_entry(entry):
    if not isinstance(entry, dict):
        return None
...
def _classify_failed_entry_source(entry):
...
def _format_failed_entry_source_label(entry):
