...
def _summarize_error_text(error_text):
...
def _summarize_recovery_text(recovery_text):
...
def _is_execution_failure_result(result):
...
def _build_recovery_suggestion(error_text, action="", user_input="", command=None):
    text = (error_text or "").strip()
    if not text:
        return u"请检查输入后重试。"
    ...
    missing_labels = _find_missing_required_params(action, command)
...
def _find_missing_required_params(action, command):
    params = (command or {}).get("params") or {}
    missing = []
...
def _log_failed_turn(
    ...
    recovery_suggestion = _build_recovery_suggestion(
        error_text,
        action=action,
