...
from ai.conversation_parser import (
    ...
    SOURCE_LABELS,
    _classify_failed_entry_source,
    _format_failed_entry_source_label,
    _load_command_entries_from_latest_conversation_log,
    _load_failed_command_entries_from_latest_conversation_log,
    _load_last_command_from_latest_conversation_log,
    ...
    _normalize_failed_filter_state,
    _normalize_failed_selected_round_index,
)
...
class ReplayCommandOption(object):
    ...
    def __init__(self, entry):
        ...
        error_summary = _shorten_text(entry.get("error_summary") or u"", limit=28)
        recovery_summary = _shorten_text(entry.get("recovery_summary") or u"", limit=24)
        user_input = _shorten_text(user_input, limit=24)

        name_parts = [
            u"第 {round_index} 轮".format(round_index=entry.get("round_index", "?")),
            status_label,
            _format_action_label(action),
            _format_failed_entry_source_label(entry),
            user_input or u"<空>",
        ]
        ...
        self.Description = u"第 {round_index} 轮 | {action} | {source} | {user_input}".format(
            ...
            source=_format_failed_entry_source_label(entry),
...
class ReplayNavigationOption(object):
    ...
    def __init__(self, direction, entry):
        ...
        round_index = entry.get("round_index", "?")
        action = _format_action_label(entry.get("action") or "unknown")
        source = _format_failed_entry_source_label(entry)
...
def _remember_last_failed_filter(chat_state, source_filter_kind=None, action=None, keyword=None):
    ...
    failed_filter = _normalize_failed_filter_state(
...
def _remember_last_failed_selected_round_index(chat_state, round_index):
    if not isinstance(chat_state, dict):
        return
    chat_state["last_failed_selected_round_index"] = _normalize_failed_selected_round_index(round_index)


def _get_last_failed_selected_round_index(chat_state):
    if not isinstance(chat_state, dict):
        return None
    return _normalize_failed_selected_round_index(
...
def _get_last_failed_filter(chat_state):
    ...
    return _normalize_failed_filter_state(
...
def _replay_last_command(
    ...
    if not last_command:
        _print_system_message(output, u"当前没有可重放的上一条指令。")
        return levels

    action = last_command.get("action", "unknown")
    _print_system_message(output, u"正在重放上一条归一化指令。")
    output.print_md("```json\n{}\n```".format(_format_command_text(last_command)))

    try:
        result, levels = _execute_command(doc, last_command, levels)
...
def _replay_last_command_from_log(
    doc,
    output,
    levels,
    operation_log,
    ...
    command = _load_last_command_from_latest_conversation_log()
    if not command:
        _print_system_message(output, u"未找到可回放的最近会话文件。")
        return levels

    chat_state["last_command"] = command
    action = command.get("action", "unknown")
    _print_system_message(output, u"正在从最近一次会话文件重放上一条归一化指令。")
...
def _replay_pick_command_from_log(
    doc,
    output,
    levels,
    operation_log,
    ...
    if not entries:
        _print_system_message(output, u"最近一次会话文件中没有可选的回放指令。")
        return levels
    ...
    if not selected:
        _print_system_message(output, u"已取消历史指令重放。")
        return levels

    command = selected.entry.get("command")
    if not command:
        _print_system_message(output, u"所选历史记录缺少可执行指令。")
        return levels

    chat_state["last_command"] = command
    action = command.get("action", "unknown")
    _print_system_message(output, u"正在按轮次重放历史归一化指令。")
    output.print_md("```json\n{}\n```".format(_format_command_text(command)))
...
def _replay_pick_failed_command_from_log(
    doc,
    output,
    levels,
    operation_log,
    ...
    entries = _load_failed_command_entries_from_latest_conversation_log()
    if not entries:
        _print_system_message(output, u"最近一次会话文件中没有可选的失败指令。")
        return levels
    ...
    if not selected:
        _print_system_message(output, u"已取消失败历史指令重放。")
        return levels
    ...
    command = selected.entry.get("command")
    if not command:
        _print_system_message(output, u"所选失败记录缺少可执行指令。")
        return levels
...
def _replay_pick_failed_command_from_last_filter(
    doc,
    output,
    levels,
    operation_log,
    ...
    if not filter_state:
        filter_state = _load_last_failed_filter_from_latest_conversation_log()
        if not filter_state:
            _print_system_message(output, u"当前没有可复用的失败筛选条件，请先执行一次 /replayfail。")
            return levels
    ...
    return _replay_pick_failed_command_from_log(
        doc,
        output,
        levels,
        operation_log,
...
def _replay_adjacent_failed_command(
    doc,
    output,
    levels,
    operation_log,
    ...
    if not filter_state or current_round_index is None:
        ...
        if restored_filter_state is None:
            restored_filter_state = _load_last_failed_filter_from_latest_conversation_log()
        if restored_round_index is None:
            restored_round_index = _load_last_failed_selected_round_index_from_latest_conversation_log()

        if not restored_filter_state or restored_round_index is None:
            _print_system_message(output, u"当前没有可连续浏览的失败结果，请先执行一次 /replayfail。")
            return levels
    ...
    entries = _load_failed_command_entries_from_latest_conversation_log()
    if not entries:
        _print_system_message(output, u"最近一次会话文件中没有可用的失败指令。")
        return levels
    ...
    if current_index is None:
        _print_system_message(output, u"当前筛选结果中未找到上次重放的失败轮次，请重新执行 /replayfail。")
        return levels

    target_index = current_index + step
    if target_index < 0:
        _print_system_message(output, u"已经是当前失败筛选结果中的第一条记录。")
        return levels
    if target_index >= len(filtered_entries):
        _print_system_message(output, u"已经是当前失败筛选结果中的最后一条记录。")
        return levels
...
def _replay_last_failed_command_from_log(
    doc,
    output,
    levels,
    operation_log,
    ...
    entry = _load_last_failed_command_entry_from_latest_conversation_log()
    if not entry:
        _print_system_message(output, u"最近一次会话文件中没有可重放的失败指令。")
        return levels

    command = entry.get("command")
    if not command:
        _print_system_message(output, u"最近一次失败轮次缺少可执行指令，无法重放。")
        return levels
...
def _prepare_failed_entries_for_replay(
    ...
    if source_filter_kind:
        source_filtered_entries = _filter_failed_entries_by_source_kind(
            output,
            entries,
            source_filter_kind
        )
        ...
        source_filtered_entries, selected_source_filter_kind = _select_failed_entries_by_source(
            output,
            entries,
    ...
    if action_filter:
        filtered_entries = _filter_failed_entries_by_action_kind(
            output,
        ...
        filtered_entries, selected_action_filter = _select_failed_entries_by_action(
            output,
            source_filtered_entries,
            return_action=True
        )
    ...
    keyword_filtered_entries = _filter_failed_entries_by_keyword(
        output,
        filtered_entries,
...
def _replay_failed_entry(
    ...
    selected_round_index = _normalize_failed_selected_round_index(
        entry.get("round_index")
    )
    _remember_last_failed_selected_round_index(chat_state, selected_round_index)
    chat_state["last_command"] = command
    action = command.get("action", "unknown")
    _print_system_message(output, banner_message)
    output.print_md("```json\n{}\n```".format(_format_command_text(command)))
...
def _select_failed_entries_by_source(output, entries, return_filter_kind=False):
    sorted_entries = _sort_entries_by_recency(entries)
    source_groups = _group_entries_by_failed_source(sorted_entries)
    ...
    if not selected:
        _print_system_message(output, u"已取消失败来源筛选。")
        return None
    if return_filter_kind:
...
def _select_failed_entries_by_action(output, entries, return_action=False):
    sorted_entries = _sort_entries_by_recency(entries)
    action_groups = _group_entries_by_action(sorted_entries)
    ...
    if not selected:
        _print_system_message(output, u"已取消失败动作筛选。")
        return None
    if return_action:
...
def _filter_failed_entries_by_keyword(output, entries, filter_keyword=None, announce_match=True):
    normalized_keyword = (filter_keyword or "").strip()
    sorted_entries = _sort_entries_by_recency(entries)
...
def _filter_failed_entries_by_source_kind(output, entries, source_filter_kind):
    if not source_filter_kind:
        return _sort_entries_by_recency(entries)

    matched_entries = [
        entry for entry in _sort_entries_by_recency(entries)
        if _classify_failed_entry_source(entry) == source_filter_kind
    ]
    if matched_entries:
        return matched_entries
...
def _filter_failed_entries_by_action_kind(output, entries, action):
    if not action:
        return _sort_entries_by_recency(entries)
    ...
    _print_system_message(
        ...
        u"最近一次会话文件中没有匹配动作 `{}` 的失败指令。".format(
            _format_action_label(normalized_action)
        )
    )
    return None
...
def _infer_single_failed_source_kind(entries):
    source_kinds = set([
        _classify_failed_entry_source(entry)
...
def _format_failed_filter_summary(filter_state):
    ...
    if action:
        summary_parts.append(u"动作={}".format(_format_action_label(action)))
    if keyword:
...
def _group_entries_by_action(entries):
    groups = {}
    for entry in entries or []:
        action = entry.get("action") or u"unknown"
...
def _group_entries_by_failed_source(entries):
    groups = {}
    for entry in entries or []:
        source_filter_kind = _classify_failed_entry_source(entry)
